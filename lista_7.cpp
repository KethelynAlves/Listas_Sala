...
/*1)- Considerando o cadastro de uma agenda de endereços, nomes, idade e telefones
...
int main(){
	...
	do {
		...
		printf("Digite 3 para ver a classificaçõa em ordem alfabética.\n");
...
void cadastrar(cadastro uni[], int *n){
	setlocale(LC_ALL, "Portuguese");
	if (*n>=M){
...
void pesquisar(cadastro cad[], int n){
	...
	while(resp == 's' || resp == 'S'){
		...
		for(int i=0; i<n; i++){
			if (idaPes == cad[i].ida){
				acha=1;
				printf("Nome: %s\nIdade: %i\nEndereço: %s\nTelefone: %s", cad[i].nome, cad[i].ida, cad[i].ende, cad[i].tel);
			}
		}
	
		if (!acha){
			printf ("%d não foi localizado!", idaPes);
		}
		printf ("\nDeseja continuar a pesquisa? [S]im / [N]ão + <enter>\n");
...
void alterar(cadastro cad[], int n){
	...
	printf("Alteração do cadastro %i\n", i+1);
